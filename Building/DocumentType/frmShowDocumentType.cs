...
{
    public partial class frmShowDocumentType : MetroForm
    {
        ...
        private async Task LoadDataAsync(bool status, string search = "")
        {
            try
            {
                var list = await DocumentTypeBussines.GetAllAsync(search);
                Invoke(new MethodInvoker(() => dtBindingSource.DataSource =
        ...
        private void SetAccess()
        {
            try
            {
                ...
                btnInsert.Enabled = access?.DocumentType.Document_Type_Insert ?? false;
                btnEdit.Enabled = access?.DocumentType.Document_Type_Update ?? false;
                btnDelete.Enabled = access?.DocumentType.Document_Type_Delete ?? false;
                btnChangeStatus.Enabled = access?.DocumentType.Document_Type_Disable ?? false;
                btnView.Enabled = access?.DocumentType.Document_Type_View ?? false;
            }
            catch (Exception ex)
            {
                WebErrorLog.ErrorInstence.StartErrorLog(ex);
        ...
        public frmShowDocumentType()
        {
            InitializeComponent();
            SetAccess();
        }

        private async void frmShowDocumentType_Load(object sender, EventArgs e)
        {
        ...
        private void frmShowDocumentType_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
                switch (e.KeyCode)
