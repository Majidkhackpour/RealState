...
namespace Building.DocumentType
{
    public partial class frmShowDocumentType : MetroForm
    {
        ...
        private void LoadData(bool status, string search = "")
        {
            try
            {
                var list = DocumentTypeBussines.GetAll(search).Where(q => q.Status == status).ToList();
                dtBindingSource.DataSource =
        ...
        private void frmShowDocumentType_Load(object sender, EventArgs e)
        {
            LoadData(ST);
        }
