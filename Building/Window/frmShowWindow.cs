...
{
    public partial class frmShowWindow : MetroForm
    {
        ...
        private async Task LoadDataAsync(string search = "")
        {
            try
            {
                ...
                var list = await BuildingWindowBussines.GetAllAsync(search, _token.Token);
                BeginInvoke(new MethodInvoker(() => WindowBindingSource.DataSource =
        ...
        public frmShowWindow(bool status = true)
        {
            InitializeComponent();
            _st = status;
            SetAccess();
            DGrid.Focus();
            ucHeader.Text = "نمایش لیست انواع پنجره";
        }
        ...
        private async void mnuDelete_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (_st)
                {
                    ...
                    var prd = await BuildingWindowBussines.GetAsync(guid);
            ...
            {
                if (res.HasError)
                    this.ShowError(res, "خطا در تغییر وضعیت نوع کفپوش");
        ...
        private async void mnuView_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (DGrid.CurrentRow == null) return;
                var guid = (Guid)DGrid[dgGuid.Index, DGrid.CurrentRow.Index].Value;
                var obj = await BuildingWindowBussines.GetAsync(guid);
                if (obj == null) return;
                var frm = new frmWindowMain(obj, true);
                frm.ShowDialog(this);
            }
            catch (Exception ex)
            {
        ...
        private async void mnuEdit_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                var obj = await BuildingWindowBussines.GetAsync(guid);
                if (obj == null) return;
                var frm = new frmWindowMain(obj, false);
        ...
        private async void mnuAdd_Click(object sender, EventArgs e)
        {
            try
            {
                var frm = new frmWindowMain(new BuildingWindowBussines(), false);
                if (frm.ShowDialog(this) == DialogResult.OK)
                    await LoadDataAsync();
            }
            catch (Exception ex)
