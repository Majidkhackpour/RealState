    ...
    public partial class frmShowBuildingType : MetroForm
    {
        ...
        private async Task LoadDataAsync(string search = "")
        {
            try
            {
                ...
                var list = await BuildingTypeBussines.GetAllAsync(search, _token.Token);
        ...
        public frmShowBuildingType(bool status = true)
        {
            InitializeComponent();
            _st = status;
            SetAccess();
            DGrid.Focus();
            ucHeader.Text = "نمایش لیست انواع ملک";
        }

        private async void frmShowBuildingType_Load(object sender, EventArgs e) => await LoadDataAsync();
        ...
        private async void mnuView_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                var obj = await BuildingTypeBussines.GetAsync(guid);
                if (obj == null) return;
                var frm = new frmBuildingTypeMain(obj, true);
                frm.ShowDialog(this);
            }
            catch (Exception ex)
            {
        ...
        private async void mnuEdit_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                var obj = await BuildingTypeBussines.GetAsync(guid);
                if (obj == null) return;
                var frm = new frmBuildingTypeMain(obj, false);
        ...
        private async void mnuAdd_Click(object sender, EventArgs e)
        {
            try
            {
                var frm = new frmBuildingTypeMain(new BuildingTypeBussines(), false);
                if (frm.ShowDialog(this) == DialogResult.OK)
                    await LoadDataAsync();
            }
            catch (Exception ex)
