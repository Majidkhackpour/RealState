    ...
    public class BuildingConditionPersistenceRepository : IBuildingConditionRepository
    {
        public async Task<bool> CheckNameAsync(string _connectionString, string name, Guid guid)
        {
            try
            {
                ...
                {
                    var cmd = new SqlCommand("sp_BuildingCondition_CheckName", cn) { CommandType = CommandType.StoredProcedure };
        ...
        public async Task<List<BuildingConditionBussines>> GetAllNotSentAsync(string connectionString)
        {
            var list = new List<BuildingConditionBussines>();
            try
            {
                ...
                {
                    var cmd = new SqlCommand("sp_BuildingConditions_GetAllNotSent", cn) { CommandType = CommandType.StoredProcedure };
                    await cn.OpenAsync();
                    var dr = await cmd.ExecuteReaderAsync();
        ...
        public async Task<ReturnedSaveFuncInfo> SetSaveResultAsync(string connectionString, Guid guid, ServerStatus status)
        {
            ...
            try
            {
                using (var cn = new SqlConnection(connectionString))
                {
                    var cmd = new SqlCommand("sp_BuildingConditions_SetSaveResult", cn)
        ...
        public async Task<ReturnedSaveFuncInfo> ResetAsync(string connectionString)
        {
            ...
            try
            {
                using (var cn = new SqlConnection(connectionString))
                {
                    var cmd = new SqlCommand("sp_BuildingConditions_Reset", cn)
                    { CommandType = CommandType.StoredProcedure };
                    await cmd.ExecuteNonQueryAsync();
                }
            }
