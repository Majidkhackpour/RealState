...
namespace EntityCache.SqlServerPersistence
{
    public class BuildingOptionPersistenceRepository : IBuildingOptionRepository
    {
        ...
        public async Task<List<BuildingOptionsBussines>> GetAllAsync(string _connectionString, CancellationToken token)
        {
        ...
        public async Task<List<BuildingOptionsBussines>> GetAllNotSentAsync(string connectionString)
        {
            var list = new List<BuildingOptionsBussines>();
            try
            {
                ...
                {
                    var cmd = new SqlCommand("sp_BuildingOption_GetAllNotSent", cn) { CommandType = CommandType.StoredProcedure };
                    await cn.OpenAsync();
                    var dr = await cmd.ExecuteReaderAsync();
        ...
        public async Task<ReturnedSaveFuncInfo> SetSaveResultAsync(string connectionString, Guid guid, ServerStatus status)
        {
            ...
            try
            {
                using (var cn = new SqlConnection(connectionString))
                {
                    var cmd = new SqlCommand("sp_BuildingOption_SetSaveResult", cn)
