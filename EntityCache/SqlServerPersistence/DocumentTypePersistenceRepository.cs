...
namespace EntityCache.SqlServerPersistence
{
    public class DocumentTypePersistenceRepository : IDocumentTypeRepository
    {
        ...
        public async Task<List<DocumentTypeBussines>> GetAllAsync(string _connectionString, CancellationToken token)
        {
        ...
        public async Task<List<DocumentTypeBussines>> GetAllNotSentAsync(string connectionString)
        {
            var list = new List<DocumentTypeBussines>();
            try
            {
                ...
                {
                    var cmd = new SqlCommand("sp_DocType_GetAllNotSent", cn) { CommandType = CommandType.StoredProcedure };
                    await cn.OpenAsync();
                    var dr = await cmd.ExecuteReaderAsync();
        ...
        public async Task<ReturnedSaveFuncInfo> SetSaveResultAsync(string connectionString, Guid guid, ServerStatus status)
        {
            ...
            try
            {
                using (var cn = new SqlConnection(connectionString))
                {
                    var cmd = new SqlCommand("sp_DocType_SetSaveResult", cn)
        ...
        public async Task<ReturnedSaveFuncInfo> ResetAsync(string connectionString)
        {
            ...
            try
            {
                using (var cn = new SqlConnection(connectionString))
                {
                    var cmd = new SqlCommand("sp_DocType_Reset", cn)
                    { CommandType = CommandType.StoredProcedure };
                    await cn.OpenAsync();
                    await cmd.ExecuteNonQueryAsync();
                    cn.Close();
