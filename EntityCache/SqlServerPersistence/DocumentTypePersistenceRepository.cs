...
namespace EntityCache.SqlServerPersistence
{
    public class DocumentTypePersistenceRepository : GenericRepository<DocumentTypeBussines, DocumentType>, IDocumentTypeRepository
    {
        ...
        public override async Task<List<DocumentTypeBussines>> GetAllAsync()
        {
        ...
        public override async Task<ReturnedSaveFuncInfo> SaveAsync(DocumentTypeBussines item, string tranName)
        {
            ...
            try
            {
                using (var cn = new SqlConnection(_connectionString))
                {
                    var cmd = new SqlCommand("sp_DocumentType_Save", cn) { CommandType = CommandType.StoredProcedure };
        ...
        public override async Task<ReturnedSaveFuncInfo> SaveRangeAsync(IEnumerable<DocumentTypeBussines> items, string tranName)
        {
            var res = new ReturnedSaveFuncInfo();
            try
            {
        ...
        public override async Task<ReturnedSaveFuncInfo> ChangeStatusAsync(DocumentTypeBussines item, bool status, string tranName)
        {
            var res = new ReturnedSaveFuncInfo();
            try
            {
                using (var cn = new SqlConnection(_connectionString))
                {
                    var cmd = new SqlCommand("sp_DocumentType_ChangeStatus", cn) { CommandType = CommandType.StoredProcedure };
                    cmd.Parameters.AddWithValue("@Guid", item.Guid);
